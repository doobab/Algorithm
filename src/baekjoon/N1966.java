...
public class N1966 {
    public static void main(String[] args){
        ...
        for (int t = 1; t <= tc; t++){
            ...
            // 순서 찾기
            for (int i = 0; i < n; i++){
                point = mostImportant(importance, print, point);
                print[point] = true;
                rank++;
                if (point == m)
                    break;
    ...
    // 남아 있는 문서 중 가장 중요한 문서 위치
    static int mostImportant(int[] importance, boolean[] print, int start) {
        int most = 0;
        int most_index = -1;
        for (int i = start; i < start + importance.length; i++){
            int index = i % importance.length;
            if (!print[index] && most < importance[index]){
                most = importance[index];
                most_index = index;
            }
        }
        return most_index;
